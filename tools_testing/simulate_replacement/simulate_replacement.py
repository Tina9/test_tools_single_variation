...
def info_sorting(sum_info):
    ...
    rep_sum_info = "./data/replacement_info.txt"
...
def main_replacement(reads_length, multiple_count, single_count, bp, repeat_time):

    sum_info = {}
    for ti in range(int(repeat_time)):
        raw_fasta_prefix = "raw_insertion_" + str(ti + 1)
        raw_fasta = produce_fasta(raw_fasta_prefix)
        multiple_output = "/".join(raw_fasta.split("/")[:2]) + "/multiple" + '_' + str(ti+1) + '_' + str(bp) + "bp" + "_"
        single_output = "/".join(raw_fasta.split("/")[:2]) + "/single" + '_' + str(ti+1) + '_' + str(bp) + "bp" + "_"
        ...
        rep_sum_info = info_sorting(sum_info)

        ### add tools to be tested here ###

        ###################################

    return sum_info, rep_sum_info
...
def parse_parameters(arguments):
    ...
    main_replacement(reads_length, multiple_count, single_count, bp, repeat_time)

if __name__ == "__main__":
    usage = """
    Usage:
        simulate_replacement.py [-l=150] [-c=100000] [-b=1] [-t=1] MULTIPLE
